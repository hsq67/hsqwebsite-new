...
// Price ranges for each category (₹ per night)
const PRICE_RANGES: Record<RoomCategory, { min: number; max: number } | null> =
  {
    All: null,
    Standard: { min: 11200, max: 14000 },
    "Duluxe-Plus": { min: 25000, max: 28000 },
    Deluxe: { min: 16000, max: 20000 },
    Executive: { min: 24000, max: 30000 },
    Presidential: { min: 28800, max: 36000 },
  };
...
const RoomsRibbon: React.FC<RoomsRibbonProps> = ({
  ...
  return (
    <div className="w-full mx-auto mb-8 px-0 md:px-6">
      ...
      {/* Filter Tabs - Mobile: 3 columns aligned left */}
      <div className="relative flex md:justify-center">
        <div className="grid grid-cols-2 md:flex md:flex-row md:flex-wrap md:justify-center gap-2 sm:gap-3 w-full">
          {CATEGORIES.map((category) => {
            const isActive = activeCategory === category;
            ...
            return (
              ...
              >
                ...
                <span className="relative z-10 flex flex-col items-center justify-center gap-0 whitespace-nowrap">
                  {/* Category name + active dot */}
                  ...
                  {/* Price range (hide for "All") */}
                  {priceRange && (
                    <span className="text-[0.6rem] sm:text-[0.7rem] md:text-xs poppins-semibold opacity-80 mt-0.5">
                      PKR{priceRange.min.toLocaleString()} – PKR
