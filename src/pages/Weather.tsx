...
import { useEffect, useRef, lazy, Suspense } from "react";
import { useQuery } from "@tanstack/react-query";
import { format } from "date-fns"; // 👈 added for month abbreviation
const Lottie = lazy(() => import("lottie-react"));
...
function Weather() {
  ...
  // ✅ Updated to use date-fns format for abbreviated month
  const getNextDateFormatted = (addDays: number) => {
    const newDate = new Date();
    newDate.setDate(date.getDate() + addDays);
    return format(newDate, "dd MMM yyyy"); // e.g., "15 Mar 2025"
  };
      ...
      <section className="backgroundcolor">
        ...
        >
          ...
          <header className="text-center">
            <h1 className="text-xl poppins-semibold text-black 2xl:text-2xl">
              Murree, Punjab | <br className="block sm:hidden" /> {dayName},{" "}
              {/* ✅ Replaced manual date with date-fns format */}
